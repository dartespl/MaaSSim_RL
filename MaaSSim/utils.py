...
def rand_node(df):
    ...
    return df.loc[random.choice(list(df.index))].name
...
def generate_platforms(_inData, _params, nPM): #f#
    ...
    plats['isMarketingOn'] = _params.platforms.isMarketingOn
    return plats    
...
def generate_demand(_inData, _params=None, avg_speed=False):
    ...
    # requests['walkDistance'] = requests['dist'] * 0.7 #my 1
    # print(requests) #my 2
    # requests['u_PT'] = -3 - (requests['dist'] / 900.0) #my 1
    # requests['u_PT'] = -3 - (requests['dist'] / 800.0) #my 1
    # _inData.passengers['u_PT'] = requests.u_PT
    # print(_inData.passengers) #my 2
...
def read_requests_csv(_inData,_params, path): #f#
    ...
    # print(_inData.skim)
    # print(_inData.skim.columns.tolist())
    # print(_inData.skim.index.tolist())
    # df = df[df.origin in _inData.skim.index.tolist()]
    # df = df[df.destination in _inData.skim.columns.tolist()]
    ...
    def getDist(request):
        # try:
        x = _inData.skim.loc[request.origin, request.destination]
        return x
        # except:
        #     print(request.origin, request.destination)
        #     return float("NAN")
        
    requests.treq = [_params.t0 + pd.Timedelta(int(_), 's') for _ in treq]
    # print("4.1", str(_inData.skim))
    # print("\n\n" + str(requests['origin']))
    # print("\n\n" + str(requests['destination']))
    requests['dist'] = requests.apply(getDist, axis=1)
    # print(requests['dist'])
    # requests['dist'] = requests.apply(lambda request: _inData.skim.loc[request.origin, request.destination], axis=1)
    # print("4.2")
